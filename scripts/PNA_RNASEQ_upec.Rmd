---
title: "RNAseq analzsis on upec data"
author: "Jakob Jung"
...
Now we do RUVg using the least significantly changing genes as negative controls and create RLE and PCA plots again. We look at the plots for all different k using a loop (output not shown here, saved as pdf):
```{r, echo=F, results = 'hide'}
for (k in 1:(length(test)-1)) {   
      par(mfrow=c(1,2))
      set_RUVg <- RUVg(set, empirical, k=k)
     plotRLE(set_RUVg, outline=FALSE, ylim=c(-1, 1), col=colors[test],
             main=paste(c("k = ", k)))
      plotPCA(set_RUVg, col=colors[test], cex=1.2, main=paste(c("k = ", k)))
}

```
k=3 improves the TMM PCA plot, so we will compare the results of both methods. 

# Differential Expression analysis using RUVg
we use RUVg normalization now with k=3:
```{r, echo=F}
k = 3
set_RUVg <- RUVg(set, empirical, k=k)
plotRLE(set_RUVg, outline=FALSE, ylim=c(-1, 1), col=colors[test],
        main=paste(c("k = ", k)))
plotPCA(set_RUVg, col=colors[test], cex=1)

plotRLE(set_RUVg, col=colors[test], cex=9,labels=F, outline=FALSE, ylim=c(-1, 1), xaxt="none")
legend("topleft", legend = levels(test),  col=colors,  pch=rep(15,3), ncol=6 )

# save as svg:
svg("../analysis/PCA_RUVg.svg")
plotPCA(set_RUVg, col=mycolors[test], cex=1)
dev.off()

#only for acpp:
mycolors <- colorRampPalette(brewer.pal(3, "Set1"))(3)
a <- normCounts(set_RUVg)


a_t <- a[, grepl("acpP|control_\\d$", colnames(a))]
t_a <- as.factor(gsub("(*)_\\d", "\\1", colnames(a_t)))
plotPCA(a_t, col=mycolors[t_a])


svg("../analysis/PCA_acpp.svg")
plotPCA(a_t, col=mycolors[t_a], xlim = c(-0.6, 0.6))
dev.off()

mycolors <- colorRampPalette(brewer.pal(8, "Set1"))(16)

a_alle <- a[, !grepl("adk|tnaB|rnpA", colnames(a))]
t_allex <- as.factor(gsub("_\\d[ab]?", "", colnames(a_alle)))

svg("../analysis/PCA_all.svg")
plotPCA(a_alle, col=mycolors[t_allex], xlim = c(-0.4, 0.4))
dev.off()
...
do_volcano <- function(restab, targetgene = NULL, pointsize = 2, x_limit = F,y_limit=F, show_sig = F, alpha=0.05, 
  ...
  cols = c("target"="red","0 mm"="cyan", "1 mm"="blue","2 mm"="darkblue", "PhoPQ"="darkred")
...
```{r}
topDEgenes <- unique(unlist(lapply(all_res_tmm, function(x) c(rownames(topTags(x, n=5)), rownames(topTags(x, n=5, sort.by = "logFC"))))))
